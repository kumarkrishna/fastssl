...
def gen_image_label_pipeline_ffcv_multiview(
    train_dataset=None,
    val_dataset=None,
    batch_size=32,
    num_workers=24,
    ...
    for split in ["train"]:
        ...
        for view_idx in range(num_views):
            image_pipeline = gen_image_pipeline_ffcv_test(
                device=device, transform_cls=transform_cls, rescale=rescale
            )
            pipelines[f"image{view_idx}"] = image_pipeline
        ordering = OrderOption.RANDOM  # if split == 'train' else OrderOption.SEQUENTIAL
        loaders[split] = Loader(
            datadir[split],
            batch_size=batch_size,
...
def gen_image_label_pipeline_ffcv_test(
    ...
    for split in ["train"]:
        ...
        loaders[split] = Loader(
            ...
            order=ordering,
            drop_last=False,
            pipelines={"image1": image_pipeline1, "image2": image_pipeline2}
...
def cifar_ffcv_multiview(
    train_dataset=None, val_dataset=None, batch_size=32, num_workers=4, device="cuda:0"
):
    transform_cls = CifarTransformMultiViewFFCV()
    return gen_image_label_pipeline_ffcv_multiview(
        train_dataset=train_dataset,
        val_dataset=val_dataset,
        batch_size=batch_size,
        num_workers=num_workers,
