...
from fastssl.models import linear, byol, simclr, vicreg, mae
...
SSL_ALGORITHMS = ["BarlowTwins", "SimCLR", "ssl", "byol", "VICReg", "mae"]
...
Section("training", "Fast CIFAR-10 training").params(
    ...
    mask_ratio=Param(float, "Mask ratio for MAE", default=0.8),
)
...
def build_dataloaders(
    ...
    if "cifar" in dataset:
        if algorithm in SSL_ALGORITHMS:
            # return cifar_pt(
    ...
    for inp in train_bar:
        ...
        if scaler:
            with autocast():
                ...
                elif args.algorithm in SSL_ALGORITHMS:
...
def run_experiment(args):
    ...
    valid_args = args.eval

    set_seeds(train_args.seed)
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    if valid_args.use_precache:
        search_precache_file(train_args, valid_args)
    ...
    loaders = build_dataloaders(
        ...
        train_args.datadir,
        train_args.train_dataset,
        train_args.val_dataset,
        train_args.batch_size,
        train_args.num_workers,
        train_args.num_augmentations,
    )
    ...
    if train_args.precache:
        ...
        # get loss function
        loss_fn = build_loss_fn(train_args)
...
if __name__ == "__main__":
    ...
    # get logging model name and type, setup logging
    ...
    if args.logging.use_wandb:
        start_wandb_server(
            train_config_dict=args.training.__dict__,
