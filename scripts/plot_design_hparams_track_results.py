    ...
    try:
        ...
        if hparam_filter_val > 0:
            if hparam_filter_condition != hparam_filter_val:
                continue
            if ssl_alg == "simclr":
                if temp_val not in tmp_vals:
                    continue
                if bsz_val not in bsz_vals:
                    continue
                if pdim_val not in pdim_vals:
                    continue
            elif ssl_alg == "barlowtwins":
                if lamda_val not in lamda_vals:
                    continue
                if pdim_val not in pdim_vals:
                    continue
            else:
        ...
        if verbose:
            if ssl_alg == "barlowtwins":
                tqdm.write(
                    "lamda = {}, pdim = {}, alpha = {}, accuracy = {}".format(
                        lamda_val,
                        pdim_val,
                ...
                tqdm.write(
                    "temp = {}, pdim = {}, bsz = {}, alpha = {}, accuracy = {}".format(
                        temp_val,
                        pdim_val,
                        bsz_val,
                        list(epoch_alpha_dict.values())[-1],
...
for epoch_alpha, final_accuracy in res_all_files:
    color_idx = int(
        np.round(N_colors * (final_accuracy - min_acc_cbar) / accuracy_range)
    )
    plt.figure(r"Scatter plot: $\alpha$ vs accuracy")
