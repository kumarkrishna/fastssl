...
from fastssl.data import (
    ...
    stl_classifier_ffcv,
    simple_dataloader,
    imagenet1k_ffcv_dist as imagenet_ffcv,
...
def train_step(
    ...
    for inp in train_bar:
        ...
        if lrd_scheduler:
            ...
            #     tqdm.write(f"{lrd_scheduler.get_last_lr()}")
            # NOTE:
            # Might throw warnings about calling scheduler.step() before opt.step()
            # See here: https://discuss.pytorch.org/t/optimizer-step-before-lr-scheduler-step-error-using-gradscaler/92930
...
def lr_lambda(step: int, warmup_steps: int=10, total_steps: int=100, end_lr_ratio: float=0.001):
    if step < warmup_steps:
        # Linear warmup
        return step / warmup_steps
    else:
        # Cosine annealing after warmup
        cosine_factor = 0.5 * (1 + math.cos(math.pi * (step - warmup_steps) / (total_steps - warmup_steps)))
        return (1-end_lr_ratio)*cosine_factor + end_lr_ratio
