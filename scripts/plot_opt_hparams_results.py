...
def plot_scatterplot(attribute1_dict,attribute1_label,attribute2_dict,attribute2_label,filter_dict=None,vmin=None,vmax=None,hmin=None,hmax=None):
	...
	for pidx,pdim in enumerate(attribute1_dict.keys()):
		...
		plt.errorbar(x=attr1_arr,y=attr2_arr,xerr=attr1_err_arr,yerr=attr2_err_arr,marker=markers_arr[pidx%len(markers_arr)],ls='',label='wd={:.1e}'.format(pdim))
...
sorting_order = [i[0] for i in sorted(enumerate(files),key=lambda x: float(os.path.basename(x[1]).split('lr_')[-1].split('_')[0])*1e6+float(os.path.basename(x[1]).split('wd_')[-1].split('_')[0]))]
...
for fidx,file in enumerate(tqdm(files_sorted)):
	try:
		lr_val = float(os.path.basename(file).split('lr_')[-1].split('_')[0])
		wd_val = float(os.path.basename(file).split('wd_')[-1].split('_')[0])
		if wd_val not in accuracy_dict.keys():
			accuracy_dict[wd_val] = {}
		if wd_val not in best_accuracy_dict.keys():
			best_accuracy_dict[wd_val] = {}
		if wd_val not in alpha_dict.keys():
			alpha_dict[wd_val] = {}
		if wd_val not in SSL_loss_dict.keys():
			SSL_loss_dict[wd_val] = {}
		if wd_val not in R2_dict.keys():
			R2_dict[wd_val] = {}
		if wd_val not in R2_100_dict.keys():
			R2_100_dict[wd_val] = {}
		...
		for linear_fname in linear_files:
			...
			if lr_val not in alpha_dict[wd_val].keys():
				alpha_dict[wd_val][lr_val] = []
				R2_dict[wd_val][lr_val] = []
				R2_100_dict[wd_val][lr_val] = []
				accuracy_dict[wd_val][lr_val] = []
				best_accuracy_dict[wd_val][lr_val] = []

			if (calc_new_alpha and linear_dict['R2_100']<R2_thresh) or dataset_ssl!=dataset_classifier:
			...
			if plot_abs:
				alpha_dict[wd_val][lr_val].append(np.abs(1-alpha))
			else:
				alpha_dict[wd_val][lr_val].append(alpha)
			R2_dict[wd_val][lr_val].append(R2)
			R2_100_dict[wd_val][lr_val].append(R2_100)
			test_acc_arr = np.array(linear_dict['test_acc_1'])
			# breakpoint()

			final_test_acc = test_acc_arr[-1]
			best_test_acc = test_acc_arr.max()
			accuracy_dict[wd_val][lr_val].append(final_test_acc)
			best_accuracy_dict[wd_val][lr_val].append(best_test_acc)
			lr_arr[lr_val]=True
			wd_arr[wd_val]=True

			if flag_debug: 
				plot_alpha_fit(linear_dict,trange=np.arange(3,100),lamda_val=lr_val,pdim_val=wd_val)
				breakpoint()
	except Exception:
		breakpoint()
		pass
...
lr_arr = np.array(list(lr_arr.keys()))
wd_arr = np.array(list(wd_arr.keys()))
plot_colorplot(wd_arr,lr_arr,accuracy_dict,"Final accuracy",cmap='coolwarm',vmin=70,vmax=85 if 'stl' in dataset_classifier else 90)
plot_colorplot(wd_arr,lr_arr,alpha_dict,r"$|1-\alpha|$" if plot_abs else r"$\alpha$",cmap='coolwarm_r',vmax=1.2 if plot_abs else 2.2)
plot_colorplot(wd_arr,lr_arr,R2_100_dict,r"$R^2$ (top 100)",cmap='coolwarm')
plot_colorplot(wd_arr,lr_arr,SSL_loss_dict,"SSL loss (log)",cmap='coolwarm_r')#,vmax=6000)
plot_scatterplot(alpha_dict,r"$|1-\alpha|$" if plot_abs else r"$\alpha$",accuracy_dict,"Final accuracy",vmin=70,vmax=85 if 'stl' in dataset_classifier else 90,
...
# plt.figure()
# plt.plot(lr_arr,accuracy_arr,ls='--',marker='o')
# plt.xlabel(r'$\lambda$')
# plt.ylabel('Test accuracy')
# plt.xscale('log')
# plt.title('BarlowTwins trained features on {}'.format(dataset))
