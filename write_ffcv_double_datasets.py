...
class DoubleImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.transform is not None:
            return self.transform(self.dataset[idx][0]), self.transform(
                self.dataset[idx][0]
            )
...
if write_dataset:
    ...
    double_data = DoubleImageDataset(
...
loader = Loader(
    ...
    # os_cache=True,
    drop_last=False,
    pipelines={"image1": image_pipeline, "image2": image_pipeline},
)
...
double_data = DoubleImageDataset(
    dataset=dataset, dataset_folder=dataset_folder, transform=transform_tv
)
...
print("Dataset mean/255", mean / 255.0)
### CIFAR stats
...
assert torch.allclose(tv_mean, mean / 255.0), "Means are different"
assert torch.allclose(tv_std, std / 255.0), "Stdev are different"
### CIFAR stats
